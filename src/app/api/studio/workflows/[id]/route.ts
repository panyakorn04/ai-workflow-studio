...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!id || id.length > 128) {
    return NextResponse.json({ ok: false, error: { message: "Invalid workflow id." } }, { status: 400 });
  }
  const target = studioBackendURL(`/api/admin/studio/workflows/${encodeURIComponent(id)}`);
  if (!target) {
