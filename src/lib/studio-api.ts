import { executions as fallbackExecutions, loopStages as fallbackStages, workflows as fallbackWorkflows } from "./demo-data";
import type { ExecutionStatus, WorkflowStatus } from "./workflows";

export type StudioWorkflow = {
  id: string; name: string; description: string; category: string; status: WorkflowStatus;
  runs: number; success: number; updated: string; nodes: string[];
};
export type StudioExecution = {
  id: string; workflow: string; status: ExecutionStatus; started: string;
  duration: string; durationMs: number; cost: number;
};
export type StudioStage = { name: string; detail: string; state: "done" | "running" | "pending" };
export type StudioOverview = {
  workflows: StudioWorkflow[]; executions: StudioExecution[]; stages: StudioStage[];
...
function isStage(value: unknown): value is StudioStage {
  if (!isRecord(value)) return false;
  return typeof value.name === "string" && typeof value.detail === "string" &&
...
export function parseStudioOverview(payload: unknown): StudioOverview {
  ...
  if (!Array.isArray(workflows) || !Array.isArray(executions) || !Array.isArray(stages) ||
      workflows.length === 0 || executions.length === 0 || stages.length === 0 ||
      !workflows.every(isWorkflow) || !executions.every(isExecution) || !stages.every(isStage)) {
    throw new Error("Invalid studio overview response");
  }
